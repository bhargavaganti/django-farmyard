...
from farmyard.utils import get_fancy_time, unique_slugify, google_lat_long
...
class USZipcodeField(models.CharField):
    ...
    A really simple field that just makes sure to pad US zipcodes with zeros
    if needed.
    '''
...
class Farm(TitleSlugDescriptionModel, TimeStampedModel):
    """
    Farm model class. Info about a farm
    """
    contact = models.CharField(_('Contact'),
                               max_length=200, **bnull)
    active = models.BooleanField(_('Active'),
                                 help_text='Is this your farm?', default=False)
    address = models.CharField(_('Address'), max_length=255)
    town = models.CharField(_('Town'), max_length=100)
    state = USStateField(_('State'))
    zipcode = USZipcodeField(_('Zip'), max_length=5)
    phone = PhoneNumberField(_('phone'), **bnull)
    lat_long = models.CharField(_('Coordinates'), max_length=255, **bnull)
    ...
    def save(self, *args, **kwargs):
        if not self.lat_long:
            logger.debug("Looking up latitude and longitude for {0} {1}, {2}"
                         .format(self.address, self.town, self.state))
            location = "%s +%s +%s +%s" % (self.address, self.town, self.state,
                                           self.zipcode)
            self.lat_long = google_lat_long(location)
            if not self.lat_long:
                location = "%s +%s +%s" % (self.town, self.state, self.zipcode)
                self.lat_long = google_lat_long(location)
            logger.debug("Latitude and longitude set to {0} for {1} {2}, {3}"
                         .format(self.lat_long, self.address, self.town,
                                 self.state))
...
class Genus(TitleSlugDescriptionModel, TimeStampedModel):
    ...
    """
    plural_name = models.CharField(_('Plural name'), max_length=200,
                                   help_text="Only use if adding an 's' does \
                                              not work.",
                                   **bnull)
    technical_name = models.CharField(_('Technical title'),
                                      max_length=200, **bnull)
...
class Breed(TitleSlugDescriptionModel, TimeStampedModel):
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('fm-breed-detail', None,
                {'slug': self.slug, 'genus_slug': self.genus.slug})
...
class Animal(TimeStampedModel):
    ...
    sire = models.ForeignKey('self', related_name="sire_",
                             blank=True, null=True)
    breeder_farm = models.ForeignKey(Farm,
                                     related_name='breeder_farm', **bnull)
    alt_breeder = models.CharField(_('Other breeder'),
                                   blank=True, null=True, max_length=255)
    owner_farm = models.ForeignKey(Farm, related_name='owner_farm', **bnull)
    alt_owner = models.CharField(_('Other owner'), max_length=255, **bnull)
    birthday = models.DateField(_('Birthday'), **bnull)
    birthtime = models.TimeField(_('Birthtime'), **bnull)
    deathday = models.DateField(_('Deathday'), **bnull)
    description = models.TextField(_('Description'), **bnull)
    image = FilerImageField(**bnull)
    sex = models.CharField(_('Sex'), choices=SEX_CHOICES,
    ...
    def __unicode__(self):
        if self.name:
            ...
            return u'%s from %s (ID: %s)' % (self.sex,
                                             self.dam.name,
                                             self.uuid[:10])
        else:
    ...
    def display_name(self):
        if self.name:
            ...
            return u'Unnamed child of {0}'.format(self.dam.name)
        else:
            return u'Unnamed {0}'.format(self.primary_breed)
        ...
        if not self._age:
            if self.birthday:
                if self.deathday:
                    ...
                    DELTA = datetime.now()
                self._age = get_fancy_time(relativedelta(DELTA, self.birthday),
                                           True)
        return self._age
    ...
    def location(self):
        if self.owner_farm:
            return self.owner_farm
        else:
            return self.alt_owner

    @property
    def origin(self):
        if self.breeder_farm:
            return self.breeder_farm
        else:
            return self.alt_breeder
    ...
    def progeny(self):
        return Animal.onthefarm_objects.filter(models.Q(sire=self) |
                                               models.Q(dam=self))
    ...
    def lost_progeny(self):
        return Animal.objects.filter(
            models.Q(sire=self) | models.Q(dam=self)).filter(
            owner_farm__active=True).filter(deathday__isnull=False)

    def births(self):
        """
        Set the threshold for when births are consider contiguous.
        Some animals can go hours between births, leading easily into
        the next day.
        """
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('fm-animal-detail', None,
                {'slug': self.slug,
                 'breed_slug': self.primary_breed.slug,
                 'genus_slug': self.primary_breed.genus.slug})

UNIT_CHOICES = (('g', 'gallons'),
                ('l', 'liters'),
                ('cl', 'centiliters'),
                ('ml', 'mililiters'),
                ('pt', 'pints'),
...
class Milking(TimeStampedModel):
    ...
    units = models.CharField(_('Units'), choices=UNIT_CHOICES,
                             max_length=2, default='ml')
    notes = generic.GenericRelation(Note)
    ...
    def __unicode__(self):
        return u'Milking on {0} of {1}'.format(self.milking_time,
                                               self.animal.display_name)
...
class AnimalAttribute(BaseAttribute):
    ...
    def __unicode__(self):
        return u'{0} attribute of {1}' .format(self.option,
                                               self.animal.display_name)
...
class AnimalRegistration(models.Model):
    ...
    reg_id = models.CharField(_('Registration ID'), blank=True, null=True,
                              max_length=255)
